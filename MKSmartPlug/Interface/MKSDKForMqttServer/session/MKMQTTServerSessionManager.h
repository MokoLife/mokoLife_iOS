...
@class MKMQTTServerSessionManager;
/** delegate gives your application access to received messages
 */
@protocol MKMQTTSessionManagerDelegate <NSObject>

/**
 Enumeration of MQTTSessionManagerState values
 */
typedef NS_ENUM(int, MKMQTTSessionManagerState) {
...
/** gets called when the connection status changes
 @param sessonManager the instance of MKMQTTSessionManager whose state changed
...
@interface MKMQTTServerSessionManager : NSObject<MQTTSessionDelegate>

/** the delegate receiving incoming messages
 */
@property (weak, nonatomic) id<MKMQTTSessionManagerDelegate> delegate;
 ...
 */
@property (nonatomic, readonly) MKMQTTSessionManagerState state;
