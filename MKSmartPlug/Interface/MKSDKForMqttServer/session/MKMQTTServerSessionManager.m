...
@interface MKMQTTServerSessionManager()

@property (nonatomic, readwrite) MKMQTTSessionManagerState state;
...
@implementation MKMQTTServerSessionManager
...
- (id)init {
    ...
    [self updateState:MKMQTTSessionManagerStateStarting];
...
- (MKMQTTServerSessionManager *)initWithPersistence:(BOOL)persistent
...
- (void)appDidEnterBackground {
    if (self.shouldConnectInForeground) {
        __weak MKMQTTServerSessionManager *weakSelf = self;
        self.backgroundTask = [[UIApplication sharedApplication] beginBackgroundTaskWithExpirationHandler:^{
            __strong MKMQTTServerSessionManager *strongSelf = weakSelf;
...
- (void)disconnect
{
    [self updateState:MKMQTTSessionManagerStateClosing];
    [self.session close];
...
- (void)updateState:(MKMQTTSessionManagerState)newState
{
    self.state = newState;
...
- (void)handleEvent:(MQTTSession *)session event:(MQTTSessionEvent)eventCode error:(NSError *)error
{
    ...
    switch (eventCode) {
        ...
        [self updateState:MKMQTTSessionManagerStateConnected];
        break;
        }
        case MQTTSessionEventConnectionClosed:
            [self updateState:MKMQTTSessionManagerStateClosed];
        ...
        case MQTTSessionEventConnectionClosedByBroker:
            [self updateState:MKMQTTSessionManagerStateClosed];
            [self endBackgroundTask];
            if (self.state != MKMQTTSessionManagerStateClosing) {
        ...
        [self updateState:MKMQTTSessionManagerStateError];
        break;
        }
        default:
            break;
...
{
    if (self.state == MKMQTTSessionManagerStateStarting
        && self.session != nil) {
        [self updateState:MKMQTTSessionManagerStateConnecting];
