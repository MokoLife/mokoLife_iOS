...
+ (NSInteger)dateIntervalWithSourceDate:(NSString *)sourceDate
    ...
    NSDate *source = [HCKDateFormatter getDateWithString:sourceDate];
    NSDate *dev = [HCKDateFormatter getDateWithString:devDate];
...
+ (NSString *)getDateString:(NSString *)sourceDateString
    ...
    NSDate *lastDate = [HCKDateFormatter getDateWithString:sourceDateString];
    NSDate * nextDate = [lastDate dateByAddingTimeInterval:time];
    timeStr = [[HCKDateFormatter formatterWithYMD] stringFromDate:nextDate];
    return timeStr;
}
...
+ (NSArray *)getMonthBeginAndEndWithDateString:(NSString *)dateString
    ...
    NSDate *lastDate = [HCKDateFormatter getDateWithString:dateString];
    ...
    NSString *beginString = [[HCKDateFormatter formatterWithYMD] stringFromDate:beginDate];
    NSString *endString = [[HCKDateFormatter formatterWithYMD] stringFromDate:endDate];
    ...
    NSString *sysDate = [HCKDateFormatter getCurrentSystemTime];
    for (NSInteger i = 0; i < tempCount; i ++) {
        beginString = [NSDate getDateString:beginString
                                       many:1
                                 lastOrNext:YES];
        NSInteger compareResult = [NSDate compareDate:[HCKDateFormatter getDateWithString:sysDate]
                                             withDate:[HCKDateFormatter getDateWithString:beginString]];
        if (compareResult == -1) {
            break;
        }
        [dateArr addObject:beginString];
