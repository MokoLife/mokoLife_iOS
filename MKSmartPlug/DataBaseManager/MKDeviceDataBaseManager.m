...
+ (void)insertDeviceList:(NSArray <MKDeviceModel *>*)deviceList
    ...
    dispatch_async(queueInsert, ^{
        ...
        BOOL resCreate = [db executeUpdate:@"CREATE TABLE IF NOT EXISTS deviceTable (device_mac text NOT NULL, device_name text NOT NULL, device_icon text NOT NULL, device_specifications text NOT NULL, device_function text NOT NULL);"];
        if (!resCreate) {
            [db close];
            [MKDeviceDataBaseAdopter operationInsertFailedBlock:failedBlock];
            return;
        ...
        for (MKDeviceModel *model in deviceList) {
            [db executeUpdate:@"INSERT INTO deviceTable (device_mac, device_name, device_icon, device_specifications, device_function) VALUES (?, ?, ?, ?, ?);",
             model.device_mac,model.device_name,model.device_icon,model.device_specifications,model.device_function];
        }
        if (sucBlock) {
            dispatch_main_async_safe(^{
                sucBlock();
